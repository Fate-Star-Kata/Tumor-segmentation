...
export const useThemeStore = defineStore('theme', () => {
  ...
  const currentThemeObject = computed(() => {
    return themes.value.find(theme => theme.name === currentTheme.value) || themes.value[0]
  })
  ...
  // 初始化主题 - 从 localStorage 或默认值读取
  function initTheme() {
    const savedTheme = localStorage.getItem('theme')
    const documentTheme = document.documentElement.getAttribute('data-theme')
    const theme = savedTheme || documentTheme || 'light'
  ...
  // 切换到下一个主题
  function nextTheme() {
    const currentIndex = themes.value.findIndex(theme => theme.name === currentTheme.value)
  ...
  function previousTheme() {
    const currentIndex = themes.value.findIndex(theme => theme.name === currentTheme.value)
    const previousIndex = currentIndex === 0 ? themes.value.length - 1 : currentIndex - 1
    setTheme(themes.value[previousIndex].name)
  }
